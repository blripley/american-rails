// Inject the Western Canada board into index.html: the finished SVG as a
// third <template>, and the map data in the same shape the engine already
// uses for the American and Canadian boards. Idempotent — re-run after
// regenerating either.
//
//   node scripts/western/inject.mjs
...
const svg = fs.readFileSync(path.join(HERE, 'out', 'western-board-draft.svg'), 'utf8');
...
const dataTag = `<script>window.WC_MAP=${JSON.stringify(payload)};</script>`;
const tplTag = `<template id="tpl-board-wc">${svg}</template>`;

// --- splice both in, replacing any previous injection ------------------------
const swap = (marker, block) => {
  const re = new RegExp(`<!--WC:${marker}-->[\\s\\S]*?<!--/WC:${marker}-->`);
  const wrapped = `<!--WC:${marker}-->${block}<!--/WC:${marker}-->`;
  if (re.test(html)) { html = html.replace(re, wrapped); return 'replaced'; }
  // first time: put the template beside the American/Canadian ones, the data beside SCORE_CELLS
