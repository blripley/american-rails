// Build (and validate) the Canadian Rails map data from cities.json + terrain.json.
//
//   node scripts/canada/build-map.mjs
//
// Emits scripts/canada/out/canada-board-data.json, in the same shape as the
...
// ---- 5. report + emit ------------------------------------------------------
console.log('Canadian Rails 1851 — map build');
console.log('  tiles      ', Object.keys(cells).length, JSON.stringify(counts));
console.log('  cities     ', cityCells.length, `| total full ${totalFull}, shared ${totalShared} (American board: 39 cities, 128/78)`);
console.log('  hubs       ', hubs.join(', '));
console.log('  specials   ', specials.join(', '));

const out = {
  board: { width: 1700, height: 1400, note: 'Canadian Rails 1851. Same frame, geometry and scoring track as the American board; only the map field, city furniture, labels, title and panel positions differ.' },
...
fs.writeFileSync(path.join(HERE, 'out', 'canada-board-data.json'), JSON.stringify(out, null, 2) + '\n');
console.log('\nwrote scripts/canada/out/canada-board-data.json');

// Written first, then judged — space-cities.mjs needs to read the flawed board
// in order to propose the fix for it.
if (problems.length) {
  console.error('\nFAILED:');
